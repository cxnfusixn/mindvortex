...
- OSM/Overpass wyszukuje branżę w granicach administracyjnych Białołęki, Targówka, Bielan lub Warszawy; źródło i adres są zapisane. Pokrycie nie jest pełne. Zapisujemy wyłącznie firmy z własną stroną i poprawnym e-mailem. Zapisane dane OSM podlegają ODbL i wymagają atrybucji.
- Deduplikacja na poziomie domeny (bez `www`). Profile Facebook, Instagram i Booksy są odrzucane. Wspólna domena oddziałów daje jedną propozycję. Inne platformy współdzielone należy kwalifikować ręcznie.
...
- Raport udostępniany przez losowy token ważny 30 dni; noindex, no-store, no-referrer i CSP. Pobierany HTML zawiera obrazy i adnotacje, działa samodzielnie. Sam link nie daje dostępu: raport i zasoby wymagają zalogowania właściciela. Wyłączenie firmy unieważnia link.
...
Mail zawiera diagnozę i wpływ na odbiorcę; nie linkuje raportu. Raport jest wewnętrzny. Konkretne zalecenia wykonawcze i pomysły do testów są dostępne w chronionym podglądzie historii audytu oraz wewnętrznym Markdown. Propozycja nowej strony i przeglądu identyfikacji dotyczy audytów z ofertą website; nie jest twierdzeniem, że każda firma wymaga pełnej przebudowy.
