...
export function startDiscovery(store,payload){
  ...
  return store.transaction(()=>{
    // OSM has a 55-second deadline; Google uses 20 seconds + five 8-second website batches.
...
export async function executeDiscovery(directory,job){
  ...
  try{
    let last={};
    const progress=value=>{last=value;store.db.prepare("UPDATE jobs SET payload=json_set(payload,'$.progress',json(?)) WHERE id=? AND status='running'").run(JSON.stringify(value),job.id);};
    const added=await discover(store,job.payload,progress);
    progress({...last,stage:'done',message:last.stage==='checking'?`Zakończono. ${last.message}`:`Zakończono wyszukiwanie. Dodano ${added} nowych firm.`,added});
    store.finish(job.id);
  }catch(error){store.finish(job.id,error.name==='TimeoutError' ? 'Źródło firm nie odpowiedziało w wyznaczonym czasie. Spróbuj ponownie później.' : String(error.message));}
  finally{store.close();}
}
