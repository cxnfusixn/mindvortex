...
export function reportHtml(lead, imageUrl, portfolioUrl, internal = false) {
  ...
  const images = lead.screens
    ...
    .join("");
  return `<!doctype html><html lang="pl"><head><meta charset="utf-8"><meta name="viewport" content="width=device-width,initial-scale=1"><meta name="robots" content="noindex,nofollow,noarchive"><meta name="referrer" content="no-referrer"><title>${e(lead.name)} — analiza MindVortex</title><style>body{margin:0;background:#080c0a;color:#edf4ee;font:16px/1.65 system-ui,sans-serif}main{max-width:960px;margin:auto;padding:40px 24px}header{border-bottom:1px solid #35453b;padding-bottom:28px}h1{font-size:clamp(28px,5vw,48px);line-height:1.15}h2{font-size:24px}h3{font-size:19px}p,li{color:#c0cec4}a{color:#62f68b;overflow-wrap:anywhere}section{margin:36px 0}article{padding:20px;border:1px solid #35453b;margin:16px 0}svg{display:block;width:100%;max-height:1000px;background:#152019}small{color:#a7baad}strong{color:#fff}.brand{font-size:14px;letter-spacing:.15em;color:#62f68b}@media print{body{background:white;color:black}p,li,small,strong{color:black}article,svg{break-inside:avoid}}</style></head><body><main><header><div class="brand">MIND VORTEX / ANALIZA UX</div><h1>${e(lead.name)}</h1><p>${e(a.summary)}</p><small>Data: ${e(a.generatedAt)} · Pewność: ${e(a.confidence)} · Ocena heurystyczna</small></header><section><h2>Cel i zakres</h2><p>${e(a.goal)}</p><p>Nowy klient · mobile web i desktop web · ${lead.screens.length} ekranów. Metoda ux-audit: ścieżka celu, heurystyki, powtarzalne problemy i ocena całej ścieżki.</p></section><section><h2>Jak strona może być odbierana przez klienta</h2>${a.findings.length ? a.findings.map((f) => `<article><small>${e(f.id)} · Ważność ${f.severity}/4 · ${e(f.check)} · ${e(f.heuristic)}</small><h3>${e(f.title)}</h3><p><strong>Dowód (${e(f.screen)}):</strong> ${e(f.evidence)}</p><p><strong>Wpływ na cel:</strong> ${e(f.impact)}</p>${internal ? `<p><strong>Zalecenie wewnętrzne:</strong> ${e(f.recommendation)} · Nakład: ${e(f.effort)}</p>` : ""}</article>`).join("") : "<p>Brak wystarczająco udokumentowanych problemów do wskazania zmian.</p>"}</section><section><h2>Co działa dobrze</h2><ul>${a.positives.map((p) => `<li>${e(p.description)} (${e(p.screen)}, ${e(p.heuristic)})</li>`).join("")}</ul></section><section><h2>Cała ścieżka klienta</h2><p>${e(a.journey)}</p></section>${images}${technicalHtml(lead.screens)}${internal && a.opportunities?.length ? `<section><h2>Pomysły do przetestowania</h2><p>Hipotezy do rozmowy i testów, nie stwierdzone błędy.</p><ul>${a.opportunities.map(item => `<li>${e(item)}</li>`).join("")}</ul></section>` : ""}<section><h2>Ograniczenia oceny</h2><ul>${a.limitations.map((l) => `<li>${e(l)}</li>`).join("")}</ul></section><section><h2>Zastosowane ramy oceny</h2><ul>${a.coverage.map((c) => `<li>${e(c.framework)}: ${e(c.findings)}</li>`).join("")}</ul><p><a href="https://www.nngroup.com/articles/ten-usability-heuristics/">Heurystyki Nielsena</a> · <a href="https://www.w3.org/WAI/WCAG21/quickref/">WCAG 2.1</a> · <a href="https://www.behaviormodel.org/">Fogg</a></p></section><footer><h2>${a.offer === "website" ? "Nowa strona. Spójny obraz marki." : "Kierunek dalszej współpracy"}</h2><p>${a.offer === "website" ? "Te obserwacje warto rozpatrzyć jako całość: sposób prezentowania oferty, czytelność i doświadczenie na telefonie składają się na odbiór marki. Proponujemy rozmowę o przebudowie strony i spójnej warstwie wizualnej. Zakres może objąć także odświeżenie identyfikacji, jeśli potwierdzą to cele firmy i przegląd jej pozostałych materiałów." : "Dobierzemy zakres współpracy do celów firmy i potwierdzonych potrzeb: od strony i identyfikacji wizualnej po automatyzację komunikacji lub dedykowane systemy."}</p><a href="${e(portfolioUrl)}" rel="noreferrer">Zobacz portfolio MindVortex →</a><p>Patryk Pyrka · <a href="mailto:patryk.pyrka@mindvortex.pro">patryk.pyrka@mindvortex.pro</a></p></footer></main></body></html>`;
}

function technicalHtml(screens) {
 const e=escapeHtml;
 return `<section><h2>Sprawdzenia techniczne</h2><p>Odczyty DOM z przeglądarki testowej. To sygnały do sprawdzenia, nie pełny audyt dostępności ani pomiar Core Web Vitals. Brak alt wymaga sprawdzenia funkcji obrazu; liczba H1 sama nie przesądza o błędzie.</p>${screens.filter(s=>s.technical).map(s=>`<article><h3>${e(s.label)} · ${s.width}px</h3><ul>${Object.entries(s.technical).map(([key,value])=>`<li>${e(({title:"Tytuł strony",description:"Opis meta",h1Count:"Liczba H1",imagesWithoutAlt:"Obrazy bez atrybutu alt",imageCount:"Liczba obrazów",horizontalOverflow:"Przewijanie poziome",viewport:"Meta viewport",https:"HTTPS"})[key] || key)}: ${e(typeof value==="boolean"?(value?"tak":"nie"):value || "brak")}</li>`).join("")}</ul></article>`).join("")}</section>`;
}
