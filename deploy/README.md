...
Ubuntu 24.04: install deploy/mindvortex-chromium.apparmor as /etc/apparmor.d/mindvortex-chromium and load with apparmor_parser -r. It permits user namespaces only for the Playwright browser paths, so Chromium sandbox remains enabled. Verified under the worker systemd restrictions.

2026-09-13: prospecting deployed with application revision 3e2620801d46fe0a09a743a232e07fdde2a9faf9 at /opt/mindvortex/releases/20260913095122/.next/standalone. Imported a consistent SQLite snapshot (5 qualified leads, 3 audit-history records). Login, history, protected assets, worker heartbeat and sandboxed browser passed. Sending remains disabled and queue paused. Prior active release retained: /opt/mindvortex/releases/20260912170439-preview-flc/.next/standalone. To roll back, stop the prospecting worker, restore the previous current symlink and restart mindvortex; retain the database and private environment.
