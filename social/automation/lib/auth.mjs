...
export async function sendMail(subject,text){if(!process.env.SMTP_HOST||!process.env.SOCIAL_ALERT_EMAIL)throw Error('Mail configuration missing');const port=Number(process.env.SMTP_PORT||465);if(!Number.isInteger(port)||port<1||port>65535)throw Error('Invalid SMTP port');const result=await nodemailer.createTransport({host:process.env.SMTP_HOST,port,secure:port===465,requireTLS:port!==465,disableFileAccess:true,disableUrlAccess:true,connectionTimeout:10000,socketTimeout:20000,auth:{user:process.env.SMTP_USER,pass:process.env.SMTP_PASSWORD}}).sendMail({from:process.env.SMTP_FROM,to:process.env.SOCIAL_ALERT_EMAIL,subject,text});if(!result.accepted?.length)throw Error('Mail not accepted');}
