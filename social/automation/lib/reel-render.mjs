...
export async function renderReel({outputDir,heroImage,websiteVideo,sceneKeys,introHook,ffmpeg='ffmpeg'}) {
validateReelScenes(sceneKeys);
if(introHook&&(introHook.length>23||/[\r\n]/.test(introHook)))throw Error('Intro hook exceeds fixed typography');
const dir=path.resolve(outputDir),ff=ffmpeg;
await fs.mkdir(dir,{recursive:true});
const font=openSync(path.join(root,'public/fonts/mono-regular.woff2'));
const headingFont=openSync(path.join(root,'public/fonts/mono.woff2'));
...
function label(s,y,size,color,tracking=0,typeface=font){const r=typeface.layout(s),scale=size/typeface.unitsPerEm;const width=r.positions.reduce((v,p)=>v+p.xAdvance*scale+tracking,0)-tracking;if(width>936)throw Error('Intro text exceeds fixed typography');let x=(1080-width)/2;return r.glyphs.map((g,i)=>{const out=`<path fill="${color}" transform="translate(${x} ${y}) scale(${scale} ${-scale})" d="${g.path.toSVG()}"/>`;x+=r.positions[i].xAdvance*scale+tracking;return out;}).join('');}
...
await fs.writeFile(dir+'/manifest.json',JSON.stringify({template:reelTemplate,variant:introHook?'growth-hook-v1':null,sceneKeys,createdAt:new Date().toISOString()},null,2));
return dir+'/reel.mp4';
}
