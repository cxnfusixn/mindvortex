...
export async function prepareReel(day,excludeId,platform='instagram'){
 const experiment=await growthExperiment(day,platform,'reel');
 ...
 for(let attempt=0;attempt<Math.min(10,choices.length);attempt++){
  try{
   ...
   c={...c,growth:{version:experiment.version,hook:experiment.id},editorialVersion:'curiosity-v2',category:pick.category,points:c.scenes.flatMap(s=>s.lines),project:'none',source:{url:source.url,hash:source.hash,checkedAt:source.checkedAt},templateVersion:'studio-demo-v1',templateVariant:'growth-hook-v1',alt:c.scenes.map(s=>s.title+': '+s.lines.join(' ')).join('. ')};
