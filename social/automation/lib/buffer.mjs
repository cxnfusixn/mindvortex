...
export function normalizeBufferMetrics(post){
 ...
 const value=key=>Number.isFinite(raw.get(key))&&raw.get(key)>=0?raw.get(key):null;
 const seconds=key=>{const m=(post.metrics||[]).find(m=>m.type===key);if(!m||value(key)===null)return null;if(/\(sec\)/i.test(m.name||''))return value(key);if(/\(min\)/i.test(m.name||''))return value(key)*60;return null;};
 return {views:value('views'),likes:value('reactions'),comments:value('comments'),reach:value('reach'),saved:value('saves'),shares:value('shares'),averageWatchSeconds:seconds('averageTimeWatched'),totalWatchSeconds:seconds('totalTimeWatched'),collectedAt:post.metricsUpdatedAt};
}
export async function syncBufferHistory(){
 ...
 do{const result=await buffer(`query($org:OrganizationId!,$channel:ChannelId!,$after:String){posts(first:100,after:$after,input:{organizationId:$org,filter:{channelIds:[$channel],status:[sent]}}){edges{node{id text externalLink sentAt metrics{type value name unit} metricsUpdatedAt}}pageInfo{hasNextPage endCursor}}}`,{org:cfg.organizationId,channel:cfg.channelId,after});
