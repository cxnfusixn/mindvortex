import {visualHistoryFor} from './content-selection.mjs';
...
async function seenOnInstagram(asset){
 const history=(await pool.query("SELECT h.media_id,h.media_url,h.media_type,r.content AS reel_content,h.editorial_note,coalesce(h.visual_source_key,p.content->>'visualSourceKey',CASE WHEN p.content->>'project'='none' THEN 'text-only' END) AS source FROM social_history h LEFT JOIN social_posts p ON p.media_id=h.media_id LEFT JOIN social_reels r ON r.media_id=h.media_id ORDER BY h.published_at DESC")).rows;
 const revision=createHash('sha256').update(JSON.stringify(history.map(x=>[x.media_id,x.source,x.media_type,x.reel_content?.sceneKeys]))).digest('hex');
 ...
 for(let i=0;i<unknown.length;i+=5){
  ...
  const b=await r.json();if(!r.ok||b.status!=='completed')throw Error('Porównanie zdjęć chwilowo niedostępne (HTTP '+r.status+', '+(b.error?.code||b.status||'unknown')+'). Spróbuj ponownie.');
