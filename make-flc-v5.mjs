...
const sharp=require('sharp'),{openSync}=require('fontkit');
const dir='social/reels/flc/v5',ff='social/reels/runtime/imageio_ffmpeg/binaries/ffmpeg-win-x86_64-v7.1.exe';
await fs.mkdir(dir,{recursive:true});
const font=openSync('social/automation/public/fonts/mono-regular.woff2');
...
const p=spawn(ff,['-y','-i',dir+'/intro.mp4','-i','social/reels/flc/v4/website.mp4','-i','social/reels/flc/v2/flc-reel-silent.mp4','-i',dir+'/swipes.mov','-filter_complex','[2:v]trim=start=18.2:end=20,setpts=PTS-STARTPTS[o];[0:v][1:v][o]concat=n=3:v=1:a=0[c];[c][3:v]overlay=0:0:shortest=1[v]','-map','[v]','-t','10','-c:v','libx264','-preset','fast','-crf','19','-pix_fmt','yuv420p','-movflags','+faststart',dir+'/flc-reel-silent.mp4'],{windowsHide:true,stdio:'ignore'});if((await once(p,'close'))[0])throw Error('Final render failed');
const audio=(await fs.readFile('finish-reel-audio.mjs','utf8')).replace('flc/v4','flc/v5').replace('[1,3.4,5.8,7.4]','[1.8,4.2,6.6,8.2]');await fs.writeFile('finish-reel-audio-v5.mjs',audio);
console.log('v5 render ready');
